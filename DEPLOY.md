...
> SPA 回退说明：本项目**不使用** `_redirects`（新版 Cloudflare Wrangler 会报「无限循环」）。改由 `scripts/copy-404.mjs` 在 `npm run build` 时复制 `index.html` 为 `dist/404.html`（`vite.config.js` 已还原为最简，避开 Cloudflare 配置解析报错），Cloudflare Pages / GitHub Pages 遇到子路径刷新都会返回 404.html（即 SPA 首页），前端路由接管，不会 404。
